# 学年別漢字データ
# 文部科学省 学習指導要領（2020年度施行）に基づく教育漢字
# 小学1年生〜小学6年生: 1,026字
...
GRADE_KANJI_DATA = {
    ...
    'grade4': [
        ...
        '士', '氏', '史', '司', '試', '児', '治', '滋', '辞', '鹿', '失', '借', '種', '周', '祝', '順', '初', '松', '笑', '唱', 
        '焼', '象', '照', '賞', '臣', '信', '成', '省', '清', '静', '席', '積', '折', '節', '説', '浅', '戦', '選', '然', '争', 
        '倉', '巣', '束', '側', '続', '卒', '孫', '帯', '隊', '達', '単', '置', '仲', '貯', '兆', '腸', '低', '底', '停', '的', 
        '典', '伝', '徒', '努', '灯', '堂', '働', '特', '得', '毒', '熱', '念', '敗', '梅', '博', '飯', '飛', '費', '必', '票', 
        '標', '不', '夫', '付', '府', '副', '粉', '兵', '別', '辺', '変', '便', '包', '法', '望', '牧', '末', '満', '未', '脈', 
        '民', '無', '約', '勇', '要', '養', '浴', '利', '陸', '良', '料', '量', '輪', '類', '令', '冷', '例', '歴', '連', '老', 
        '労', '録', '茨', '媛', '岡', '潟', '岐', '熊', '香', '佐', '栃', '奈', '梨', '阪', '阜'
    ],
...
# 重複削除（念のため）
for grade in GRADE_KANJI_DATA:
    GRADE_KANJI_DATA[grade] = sorted(list(set(GRADE_KANJI_DATA[grade])))
...
# 小学校の漢字総数を確認するためのヘルパー関数
def get_elementary_kanji_count():
    grades = ['grade1', 'grade2', 'grade3', 'grade4', 'grade5', 'grade6']
    return sum(len(GRADE_KANJI_DATA[g]) for g in grades)

if __name__ == '__main__':
    grades = ['grade1', 'grade2', 'grade3', 'grade4', 'grade5', 'grade6']
    for g in grades:
        print(f'{g}: {len(GRADE_KANJI_DATA[g])}字')
    print(f'小学校合計: {get_elementary_kanji_count()}字')
